...
public class articleManagement {
	...
	public Scene getScene(Stage primaryStage, String userRole, String userName) {
		...
		contentBox.getChildren().addAll(label, listArticlesButton, createArticleButton, deleteArticleButton,
				backupRestoreButton, accessRights, backButton);
	...
	private void createArticle(Stage primaryStage, String userRole, String userName) {
		...
		groupsComboBox.getItems().addAll("H2 Database", "SQL Fiddle", "IntelliJ", "Eclipse");
			...
			if ("Special Group".equals(selectedGroup)) {
				...
				dialog.showAndWait().ifPresent(name -> {
					if (!name.isEmpty()) {
						...
						Alert alert = new Alert(Alert.AlertType.WARNING, "Please enter a valid group name.",
								ButtonType.OK);
						alert.showAndWait();
		...
		submitButton.setOnAction(e -> {
			if (!groupTypeComboBox.getValue().equals("General Group")) {
				...
				try { // if user is already in a group, they can't make a new group. Only articles in
						// same group.
					if (dbHelper.getSpecialAccessGroup(userName) != null
						...
						alreadyIn.setHeaderText(
								"Error: User already in special group! Cannot make new article in new special group!");
						alreadyIn.showAndWait();
					} else {
						dbHelper.updateSpecialGroup(groupTypeComboBox.getValue(), userName);
						articleDHelper.articleCreation(groupTypeComboBox.getValue(), levelComboBox.getValue(),
								groupsComboBox.getValue(), titleField.getText(), authorField.getText(),
								abstractArea.getText(), keywordsField.getText(), bodyArea.getText(),
								referencesArea.getText(), false, -1);
						...
						primaryStage.setScene(getScene(primaryStage, userRole, userName)); // Return to main scene after
																							// creating article
					}
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
			} else { // else don't change anything and just display basic info
				articleDHelper.articleCreation(groupTypeComboBox.getValue(), levelComboBox.getValue(),
						groupsComboBox.getValue(), titleField.getText(), authorField.getText(), abstractArea.getText(),
						keywordsField.getText(), bodyArea.getText(), referencesArea.getText(), false, -1);
		...
		createBox.getChildren().addAll(titleField, authorField, abstractArea, keywordsField, bodyArea, referencesArea,
				horizontalLayoutForButtons, submitButton, backButton);
	...
	/* 
	 * Private method that handles the main backup/restore actions based on user
	 * input. It prompts the user to specify either 'Backup' or 'Restore'.
	 * The method calls private methods, handleBackup() or handleRestore(), as
	 * appropriate.
	 */
	 ...
	 * Prompts the user to select a group (e.g., "H2 Database", "SQL Fiddle") and
