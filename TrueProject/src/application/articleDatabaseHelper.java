...
class articleDatabaseHelper {
	 ...
	 * special access group, ID, level, article group, title, author, abstract,
	 * keywords, body, references.
	 */
	public void createTables() throws SQLException {
		String userTable = "CREATE TABLE IF NOT EXISTS cse360article (" + "groupType VARCHAR(255), "
				+ "id INT AUTO_INCREMENT PRIMARY KEY, " + "level VARCHAR(255), " + "article_group VARCHAR(255), "
				+ "title VARCHAR(255), " + "author VARCHAR(255), " + "abstract VARCHAR(255), "
				+ "keywords VARCHAR(255), " + "body TEXT, " + "references VARCHAR(255))";
	...
	/*
	 * loadFromFile loads data from a file and is passed a boolean to determine how
	 * it does so. True will delete all existing articles and replace them with what
	 * is in the specified file. False will instead update the current database with
	 * the data from the file while skipping duplicate titles.
	 */
	public boolean loadFromFile(String file, boolean replaceAll) {
		...
		try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
			...
			String line;
			String groupType = null, level = null, group = null, title = null, author = null, abstracts = null,
					keywords = null, body = null, references = null;
			...
			while ((line = reader.readLine()) != null) {
				if (line.startsWith("Group Type: ")) {
					...
					// While updating current database, skip adding existing articles based on
					// title.
					if (!replaceAll) {
	...
	/*
	 * deleteArticle will delete an article specified by it's given title or delete
	 * article(s) by their special access group.
	 */
	public void deleteArticle(String title, String groupType) throws SQLException {
		...
		try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
			if (groupType.equals("None")) {
				...
				preparedStatement.setString(1, groupType); // Delete by group type
			}
	...
	/*
	 * viewArticle will return the article specified by the sequence number. The
	 * body and references are initially encrypted. The passed username will be
	 * checked to see if they have the correct viewing rights, then the body and
	 * references will be displayed unencrypted.
	 */
	public String viewArticle(int sequenceNumber, String userName) throws Exception {
		...
		try (PreparedStatement stmt = connection.prepareStatement(findUserQuery)) {
			...
			while (rs.next()) {
				...
				String encryptedReferences = Base64.getEncoder().encodeToString(encryptionHelper
						.encrypt(references.getBytes(), EncryptionUtils.getInitializationVector(author.toCharArray())));

				String decryptedBody = new String(encryptionHelper.decrypt(Base64.getDecoder().decode(encryptedBody),
						EncryptionUtils.getInitializationVector(author.toCharArray())));

				String decryptedReferences = new String(
				...
				{ // Depending if the user has access, it will only show the encrypted portion if
					// user does not. Else, shows the decrypted portion
					if ((userGroupType != null && groupType.equals(userGroupType))
							...
							|| (userGroupType == null && databaseHelp.isGroupInViewingRights(userName, groupType))) // if
																													// special
																													// group
																													// name
																													// ==
				...
				} else {// If this is a general group, no need to encrypt
					article.append("Sequence Number: ").append(id).append("\n").append("Level: ").append(level)
							.append("\n").append("Group: ").append(group).append("\n").append("Title: ").append(title)
							.append("\n").append("Author: ").append(author).append("\n").append("Abstract: ")
							.append(abstracts).append("\n").append("Keyword(s): ").append(keywords).append("\n")
							.append("Body: ").append(decryptedBody).append("\n").append("Reference(s): ")
							.append(decryptedReferences).append("\n").append("-------------\n");
				}
			}
		}
	...
	/*
	 * displayArticle will display all articles in short form(without the body or
	 * references) and display it with proper formatting.
	 */
	public String displayArticles() throws Exception {
		...
		while (rs.next()) {
			...
			articleDetails.append("Group Type: ").append(groupType).append("\n").append("ID: ").append(id).append("\n")
					...
					.append("Abstract: ").append(abstracts).append("\n").append("Keyword(s): ").append(keywords)
					.append("\n").append("-------------\n");
	...
	/*
	 * Creates an article with the following arguments: Special access group, level,
	 * group, title, author, abstract, keywords, body, references, a flag that
	 * designates edit or article creation, and the article ID if it exists(depends
	 * on flag).
	 */
	public void articleCreation(String groupType, String level, String group, String title, String author,
		...
		if (update == true) { // edit existing article
			sql = "UPDATE cse360article SET groupType = ?, level = ?, article_group = ?, title = ?, "
					+ "author = ?, abstract = ?, keywords = ?, body = ?, references = ? " + "WHERE id = " + "'" + id
					+ "'";
		} else { // create new article
			sql = "INSERT INTO cse360article (groupType, level, article_group, title, author, abstract, keywords, "
					+ "body, references) "
					+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
		}
	...
	/*
	 * Helper method to facilitate the 'search' function and query articles based on
	 * keywords.
	 */
	public String searchByKeywordAndLevel(String keyword, String skillLevel) throws SQLException {
		...
		StringBuilder query = new StringBuilder(
				"SELECT * FROM cse360article WHERE (keywords LIKE ? OR title LIKE ? OR author LIKE ? OR abstract LIKE ?)");
	...
	/*
	 * Simple utility method to close the article database connection after opening
	 * it.
	 */
	...
	/*
	 * Returns article data by it's sequence number where it is then fed into the
	 * article creation GUI. Method technically doesn't update article on it's own
	 * but instead helps to keep original article data persistent.
	 */
	...
	/*
	 * Returns the special access group (groupType) of the user as either general or
	 * specific special group.
	 */
	...
	/*
	 * Returns article ID based on the given title
	 */
